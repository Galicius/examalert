...
export async function initDB() {
  ...
  await query(`CREATE INDEX IF NOT EXISTS idx_slots_town ON slots(town);`);
  ...
  await query(`CREATE INDEX IF NOT EXISTS idx_questions_likes_created ON exam_questions(likes_count DESC, created_at DESC);`);
  await query(`CREATE INDEX IF NOT EXISTS idx_questions_exam_type ON exam_questions(exam_type);`);
  await query(`CREATE INDEX IF NOT EXISTS idx_questions_category ON exam_questions(category);`);
  await query(`CREATE INDEX IF NOT EXISTS idx_questions_exam_type_likes_created ON exam_questions(exam_type, likes_count DESC, created_at DESC);`);
  await query(`CREATE INDEX IF NOT EXISTS idx_questions_category_likes_created ON exam_questions(category, likes_count DESC, created_at DESC);`);
  await query(`CREATE INDEX IF NOT EXISTS idx_questions_submitted_by ON exam_questions(submitted_by);`);
